...
public class AnalzeLiabilityExportExcel {
    ...
    public  String getQuoteStr() {
        return companyName+"："+symbol+",当前股价："+quote.get(AnalzeLiabilityExcelConstants.CURRENT).getValue()+"，当日成交额："
                +String.format("%.2f", quote.get(AnalzeLiabilityExcelConstants.AMOUNT).getValue())+"亿,流通市值："
                +String.format("%.2f", quote.get(AnalzeLiabilityExcelConstants.FLOAT_MARKET_CAPITAL).getValue())+"亿，动态市盈率（TTM）："
                +quote.get(AnalzeLiabilityExcelConstants.PE_TTM).getValue()+"，\n股息率（TTM）:"
